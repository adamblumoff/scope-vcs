use scope_postgres::db::{apply_maintenance_migrations, migration_plan, verify_schema};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let command = std::env::args()
        .nth(1)
        .ok_or_else(|| anyhow::anyhow!("usage: scope-maintenance <plan|apply|verify>"))?;
    if std::env::args().nth(2).is_some() {
        anyhow::bail!("usage: scope-maintenance <plan|apply|verify>");
    }
    ...
    match command.as_str() {
        ...
        _ => anyhow::bail!("usage: scope-maintenance <plan|apply|verify>"),
    }
    Ok(())
}
