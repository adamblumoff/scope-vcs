...
#[tokio::test]
async fn repo_events_allow_public_reader_for_public_repo_without_private_version() {
    let state = test_state_with_repo();
    {
        let mut catalog = lock_catalog(&state).unwrap();
        catalog
...
#[tokio::test]
async fn repo_events_stream_redacted_visibility_changes_to_public_readers() {
    let state = test_state_with_repo();
    cache_test_jwks(&state);
    {
        let mut repo = repo_with_readme();
        repo.graph.commits[0].changes.push(FileChange {
            ...
            path: ScopePath::parse("/api/README.md").unwrap(),
            old_content: None,
            new_content: Some(source_blob("api docs")),
        });
        let mut catalog = lock_catalog(&state).unwrap();
        catalog.repositories.insert(TEST_REPO_ID.to_string(), repo);
    }
...
#[tokio::test]
async fn repo_events_close_public_stream_when_visibility_removes_public_read() {
    let state = test_state_with_repo();
    cache_test_jwks(&state);
    {
        let mut repo = repo_with_readme();
