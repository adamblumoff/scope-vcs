use api::domain::{
    ...
    store::{
        CatalogError, FirstPushToken, FirstPushTokenStatus,
...
fn member_scope_user_principal_can_write_repo() {
    ...
    assert!(app_catalog().can_push(&repo, &principal));
}
...
fn create_repository_makes_private_owner_repo_pending_first_push() {
    ...
    let principal = user_principal(TEST_OWNER_ID);
    assert!(catalog.can_read_path(&repo, &principal, &root));
    assert!(!catalog.can_push(&repo, &principal));
    assert!(!catalog.can_read_path(&repo, &Principal::public(), &root));
}
...
fn unpublished_repo_blocks_public_reads() {
    ...
    assert!(!app_catalog().can_read_path(&repo, &Principal::public(), &ScopePath::root()));
}
...
fn pending_invite_does_not_grant_private_access() {
    ...
    assert!(!app_catalog().can_read_path(&repo, &principal, &private_path));
}
