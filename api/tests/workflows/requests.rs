...
async fn public_submit_stakes_credits_and_uses_public_base() {
    ...
    let public_bearer = bearer_header_for(PUBLIC_SUBJECT, PUBLIC_EMAIL);
...
async fn working_request_is_visible_but_not_maintainer_decision_ready() {
    ...
    let resolve = api_request(
        app.clone(),
        "POST",
        &format!("/v1/repos/owner/repo/requests/{request_id}/resolve"),
        Some(&owner_bearer),
        Some(r#"{"disposition":"UsefulNotMerged","body":"Not ready."}"#),
    )
    .await;
    assert_eq!(resolve.status(), StatusCode::CONFLICT);

    let merge_body =
        format!(r#"{{"expected_main_oid":"{REQUEST_HEAD}","expected_head_oid":"{REQUEST_HEAD}"}}"#);
    let merge = api_request(
        app,
        "POST",
        &format!("/v1/repos/owner/repo/requests/{request_id}/merge"),
        Some(&owner_bearer),
        Some(&merge_body),
    )
    .await;
...
async fn public_readers_do_not_see_private_request_branches() {
    ...
    let public_summary_response =
        api_request(app.clone(), "GET", "/v1/repos/owner/repo", None, None).await;
...
async fn needs_response_respond_and_resolution_settle_public_stake() {
    ...
    let needs_response = api_request(
        app.clone(),
        "POST",
        "/v1/repos/owner/repo/requests/req_public/needs-response",
        Some(&bearer_header()),
        Some(r#"{"body":"Please add a repro."}"#),
    )
    .await;
    let status = needs_response.status();
    let body = response_json(needs_response).await;
    assert_eq!(status, StatusCode::OK, "{body}");
    assert_eq!(body["request"]["state"], "NeedsResponse");

    let respond = api_request(
        app.clone(),
        "POST",
        "/v1/repos/owner/repo/requests/req_public/respond",
    ...
    let resolve = api_request(
        app,
        "POST",
        "/v1/repos/owner/repo/requests/req_public/resolve",
        Some(&bearer_header()),
        Some(r#"{"disposition":"UsefulNotMerged","body":"Helpful, but not merging."}"#),
    )
    .await;
...
async fn clean_merge_applies_repo_update_and_resolves_as_accepted() {
    ...
    let merge_body =
        format!(r#"{{"expected_main_oid":"{main_oid}","expected_head_oid":"{request_head}"}}"#);
    let response = api_request(
        router(state.clone()),
        "POST",
        "/v1/repos/owner/repo/requests/req_merge/merge",
        Some(&bearer_header()),
        Some(&merge_body),
    )
    .await;
...
async fn public_request_merge_replays_public_delta_without_deleting_private_files() {
    let state = state_with_public_user();
    cache_test_jwks(&state);
    grant_public_credits(&state, "ledger_public_merge_grant").await;
    ...
    let merge_body =
        format!(r#"{{"expected_main_oid":"{raw_main_oid}","expected_head_oid":"{request_head}"}}"#);
    let response = api_request(
        router(state.clone()),
        "POST",
...
async fn public_request_merge_rejects_private_path_collision() {
    let state = state_with_public_user();
    cache_test_jwks(&state);
    grant_public_credits(&state, "ledger_private_collision_grant").await;
