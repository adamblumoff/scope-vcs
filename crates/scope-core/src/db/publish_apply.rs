...
use crate::domain::{
    repo_actions::{
        apply_staged_update as apply_staged_update_command,
        publish_import as publish_import_command,
        reject_staged_update as reject_staged_update_command,
    },
    store::{RepoRecord, StagedRepoUpdate, repo_id},
};
        ...
        match self.inner.as_ref() {
            MetadataStoreInner::Postgres { db, runtime } => {
                ...
                run_api_db_on(runtime, async move {
                    ...
                    let mut repo = repository_from_model(&tx, repo).await?;
                    let mutation = apply_staged_update_command(&mut repo, &user_id)?;
                    save_repo_mutation(&tx, &repo, &mutation.effects).await?;
                    tx.commit().await.map_err(ApiError::internal)?;
                    Ok(mutation.result)
                })
            ...
            MetadataStoreInner::Memory(_) => self.update(move |catalog| {
                let repo = catalog
                    ...
                    .get_mut(&repo_id)
                    .ok_or_else(|| ApiError::not_found(format!("repo {owner}/{name} not found")))?;
                let mutation = apply_staged_update_command(repo, &user_id)?;
                apply_repo_effects(catalog, mutation.effects);
                Ok(mutation.result)
            }),
        }
        ...
        match self.inner.as_ref() {
            MetadataStoreInner::Postgres { db, runtime } => {
                ...
                run_api_db_on(runtime, async move {
                    ...
                    let mut repo = repository_from_model(&tx, repo).await?;
                    let mutation = reject_staged_update_command(&mut repo, &user_id)?;
                    save_repo_mutation(&tx, &repo, &mutation.effects).await?;
                    tx.commit().await.map_err(ApiError::internal)?;
                    Ok(mutation.result)
                })
            ...
            MetadataStoreInner::Memory(_) => self.update(move |catalog| {
                let repo = catalog
                    ...
                    .get_mut(&repo_id)
                    .ok_or_else(|| ApiError::not_found(format!("repo {owner}/{name} not found")))?;
                let mutation = reject_staged_update_command(repo, &user_id)?;
                apply_repo_effects(catalog, mutation.effects);
                Ok(mutation.result)
            }),
        }
