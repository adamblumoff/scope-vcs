...
pub fn new_operator_metadata_reset_event(reason: &str, reset_at_unix: u64) -> MetadataResetEvent {
    new_metadata_reset_event(OPERATOR_RESET_TRIGGER, reason, reset_at_unix)
}
...
pub fn metadata_reset_event_from_model(
    ...
    Ok(MetadataResetEvent {
        id: model.id,
        reset_at_unix: u64::try_from(model.reset_at_unix).map_err(|_| {
            PostgresError::internal_message("metadata reset time cannot be negative")
        })?,
