...
use scope_domain::requests::RequestChangeBlock;
use sea_orm::{
...
pub async fn change_blocks_by_ids<C>(
    conn: &C,
    ids: &[String],
) -> Result<BTreeMap<String, RequestChangeBlock>, PostgresError>
where
    C: ConnectionTrait,
{
    if ids.is_empty() {
...
pub async fn change_blocks_for_request_ids<C>(
    conn: &C,
    request_ids: &[String],
) -> Result<Vec<RequestChangeBlock>, PostgresError>
where
    C: ConnectionTrait,
{
...
pub async fn insert_change_block<C>(
    conn: &C,
    value: &RequestChangeBlock,
