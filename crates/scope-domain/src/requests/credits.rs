...
pub fn grant_user_credits(
...
) -> Result<CreditAccountMutation, DomainError> {
    ...
    let balance_credits = current_balance
        ...
        .ok_or_else(|| DomainError::invalid_input("credit balance overflow"))?;
...
fn ensure_ledger_entry_id_available(
    ...
    if ledger_entries.contains_key(ledger_entry_id) {
        Err(DomainError::conflict("credit ledger entry already exists"))
    } else {
        Ok(())
    }
}

fn u32_to_i32(value: u32) -> Result<i32, DomainError> {
    i32::try_from(value).map_err(|_| DomainError::invalid_input("credit amount exceeds i32 range"))
}
