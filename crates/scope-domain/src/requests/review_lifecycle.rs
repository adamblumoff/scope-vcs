...
pub fn mark_request_ready(
    ...
    let stake = if public {
        let stake = input
            .stake_credits
            .ok_or_else(|| DomainError::invalid_input("review stake is required"))?;
    ...
    let (credit_account, ledger_entries) = if public {
        ...
        if account.balance_credits < stake {
            return Err(DomainError::conflict("insufficient review credits"));
        }
        let id = require_id(
...
fn refund(
    ...
    event_id: &str,
    now: u64,
) -> Result<(Option<UserCreditAccount>, Vec<CreditLedgerEntry>), DomainError> {
    let stake = request.current_stake_credits;
    if stake == 0 {
    ...
    let balance = account
        .balance_credits
        .checked_add(stake)
        .ok_or_else(|| DomainError::conflict("credit balance overflow"))?;
...
fn settle(
    ...
    let credit = settlement
        ...
        .ok_or_else(|| DomainError::conflict("credit settlement overflow"))?;
    let balance = account
        .balance_credits
        .checked_add(credit)
        .ok_or_else(|| DomainError::conflict("credit balance overflow"))?;
...
) -> Result<&'a UserCreditAccount, DomainError> {
    let account =
        account.ok_or_else(|| DomainError::conflict("request author credit account is missing"))?;
...
fn require_id(label: &str, id: Option<&str>) -> Result<String, DomainError> {
    let id = id.ok_or_else(|| DomainError::invalid_input(format!("{label} is required")))?;
    validate_required_id(label, id)?;
    Ok(id.to_string())
}

fn to_i32(value: u32) -> Result<i32, DomainError> {
    i32::try_from(value).map_err(|_| DomainError::conflict("credit amount exceeds i32 range"))
}
